...
function NoteList({ archieveEvent, notes, searchValue, archievedFilter, isDetail, detailButtonEvent, selectedNoteDetail }) {
    ...
    <div className="inbox__area">
      { 
        ...
        notesArchieveFiltered.length > 0 ? notesArchieveFiltered.filter(note => searchValue === '' ? note : note.title.toLowerCase().includes(searchValue.toLowerCase())).map((note) => {
          return (<NoteItem key={note.id} id={note.id} title={note.title} date={showFormattedDate(note.createdAt)} description={note.body} archieved={note.archived} archieveEvent={archieveEvent} isDetail={isDetail} detailButtonEvent={detailButtonEvent} selectedNoteDetail={selectedNoteDetail} />)
